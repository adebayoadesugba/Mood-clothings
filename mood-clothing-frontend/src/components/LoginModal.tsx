...
export function LoginModal() {
  ...
  // GOOGLE'S OWN RENDERED BUTTON: a ref to mount Google's real sign-in button into,
  // instead of a custom button that calls prompt() indirectly (see effect below).
  ...
  // FIXED: mounts Google's OWN real sign-in button (not a custom lookalike that calls
  // prompt() indirectly). This is the reliable, currently-recommended approach — the old
  // "custom button that calls .prompt()" pattern is exactly what's affected by Chrome's
  // FedCM enforcement, especially on mobile, where the popup increasingly refuses to show
  // at all. Google's real button doesn't have this problem since it IS the actual UI
  // element the browser trusts, not a proxy.
  useEffect(() => {
    if (!loginOpen || user || forgotMode) return;

    const tryRender = () => {
      const google = (window as any).google;
      if (google?.accounts?.id && googleButtonRef.current) {
        // Clear any previously rendered button first (avoids duplicates on re-render)
  ...
  }, [loginOpen, user, forgotMode, isSignUp]);
  ...
  return (
    ...
    >
      <div
        ...
        className="relative w-full max-w-md max-h-[90vh] overflow-y-auto bg-background p-6 md:p-8"
        ...
        {user ? (
                ...
                <form onSubmit={handleForgotPasswordSubmit} className="mt-6">
                  <div>
                    ...
                    <input
                      ...
                      className="mt-1 w-full border-b border-hairline bg-transparent py-0 text-sm outline-none focus:border-foreground"
                    />
                  </div>
                  <button
                    type="submit"
          ...
          <div>
            ...
            <form onSubmit={handleSubmit}>
                ...
                {isSignUp && (
                  <div>
                    ...
                    <input
                      ...
                      className="mt-1 w-full border-b border-hairline bg-transparent py-1 text-sm outline-none focus:border-foreground"
                    />
                  </div>
                )}
                ...
                <div>
                  ...
                  <div className="relative flex items-center">
                    <input
                      ...
                      className="mt-1 w-full border-b border-hairline bg-transparent py-0 pr-8 text-sm outline-none focus:border-foreground"
                    />
                    <button
                      type="button"
              ...
              {/* Google Auth Integration — Google renders its OWN real button into this
                  div (see the useEffect above). This is what actually fixes the
                  Chrome/mobile reliability issue: it's no longer a custom button that
                  calls prompt() indirectly, it's the genuine first-party Google UI. */}
